...
async function GetOrderList(userIdx) {
    ...
    if(result.status == 1) {
		let desctime=result.data[0].add_time_str?result.data[0].add_time_str.substr(0,16):''
		let desc=result.data[0].description?result.data[0].description:''
...
async function getBalance() {
    ...
    if(result.status == 0) {
        ...
        notifyStr += `\n账户${userIdx+1} ${nickname[userIdx]}: \n`
        notifyStr += `【金币总数】：${Math.round(result.user.score/100)/100}\n`
        notifyStr += `【今日收益】：${Math.round(result.user.today_score/100)/100}\n`
		let zqstatus=result.user.status==1?'正常':'异常'
		...
		console.log(`【金币总数】：${Math.round(result.user.score/100)/100}`)
    } else {
