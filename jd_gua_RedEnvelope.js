...
rebateCodes = '3IVdLOz'//$.isNode() ? (process.env.gua_redEnvelope_rebateCode ? process.env.gua_redEnvelope_rebateCode : `${rebateCodes}`) : ($.getdata('gua_redEnvelope_rebateCode') ? $.getdata('gua_redEnvelope_rebateCode') : `${rebateCodes}`);
...
function getUA(){
  ...
  rebateCode = 'yw29Go1'
  if($.index != 1){
    let arr = [rebateCodes,'yw29Go1']
    rebateCode = arr[Math.floor(Math.random() * arr.length)] || rebateCode
    if(!rebateCode) rebateCode = 'yw29Go1'
  }
